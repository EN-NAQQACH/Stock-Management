...
if (empty($_POST['Nom']) || empty($_POST['Telephone']) || empty($_POST['Adress'])) {
    ...
    $sql = "SELECT * FROM $database.client WHERE Nom=? AND Telephone=? AND Address=?";
    $req = $connexion->prepare($sql);
    $req->execute(array($nom, $telephone, $adresse));
    ...
    if ($req->rowCount() > 0) {
        ...
        $sql = "INSERT INTO $database.client (Nom, Telephone, Address) VALUES(?, ?, ?)";
        $req = $connexion->prepare($sql);
        $req->execute(array(
            $_POST['Nom'],
            $_POST['Telephone'],
